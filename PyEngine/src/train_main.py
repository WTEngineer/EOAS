...
from src.model_lib.mulnet import mulnet
...
class TrainMain:
    ...
    def _init_model_param(self):
        ...
        if self.conf.resume == True:
            model_path = self.conf.model_path + '/init.pth'
            state_dict = torch.load(model_path, map_location=self.conf.device)
            self.model.load_state_dict(state_dict)
            self.step = self.conf.start_step
            self.start_epoch = self.conf.start_epoch
    ...
    def _define_network(self):
        ...
        model = mulnet(**param).to(self.conf.device)
