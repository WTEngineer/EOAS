  ...
  .tw-flex.tw-relative.pl-safe.pr-safe
    ...
    .filter-content.filter-included.tw-w-full.tw-relative
      .tw-w-full.tw-flex.tw-flex-wrap.tw-justify-start.tw-flex-start
        v-row.tw-w-full
          v-col.tw-px-2.tw-py-2(cols="3" v-for="user in users" :key="user.id")
            .tw-bg-white.tw-shadow.tw-rounded-lg.tw-overflow-hidden.tw-mx-2.tw-my-4.tw-pb-4.tw-max-w-xs
              ...
              .tw-w-full.tw-px-10
              v-row.tw-flex.tw-w-full.tw-text-sm.tw-leading-1.tw-text-gray-500
                v-col.tw-text-left.tw-pl-10(cols="6" style="padding: 0") {{$t('name')}}:
                v-col.tw-text-left(cols="6" style="padding: 0") {{ user.name }}
              v-row.tw-flex.tw-w-full.tw-text-sm.tw-leading-1.tw-text-gray-500
                v-col.tw-text-left.tw-pl-10(cols="6" style="padding: 0") {{$t('time')}}:
                v-col.tw-text-left(cols="6" style="padding: 0") {{ user.time }}
              v-row.tw-flex.tw-w-full.tw-text-sm.tw-leading-1.tw-text-gray-500
                v-col.tw-text-left.tw-pl-10(cols="6" style="padding: 0") {{$t('action')}}:
                v-col.tw-text-left(cols="6" style="padding: 0") {{ user.action }}
              v-row.tw-flex.tw-w-full.tw-text-sm.tw-leading-1.tw-text-gray-500
                v-col.tw-text-left.tw-pl-10(cols="6" style="padding: 0") {{$t('content')}}:
                v-col.tw-text-left(cols="6" style="padding: 0") {{ user.content }}
...
import { getAdminHistory } from '../../api/users.api';
...
export default {
  ...
  methods: {
    ...
    async fetchHistory() {
      try {
        const response = await getAdminHistory(`?pageSize=10&page=${this.page}` + this.query);
    ...
    async infiniteHandler($state) {
      try {
        if (this.selectedFilter.length >= 2) {
          const from = this.selectedFilter[0].split('=')[1];
          const to = this.selectedFilter[1].split('=')[1];
          const response = await getAdminHistory(`?pageSize=8&page=${this.page}&from=${from}&to=${to+1}`);
          ...
          const response = await getAdminHistory(`?pageSize=8&page=${this.page}` + this.query);
          if (response.data.result.length) {
            this.page += 1;
            this.users = [...this.users, ...response.data.result];
            this.originalData = [...this.originalData, ...response.data.result];
